document.addEventListener('DOMContentLoaded', function() {
        ...
        switch(step) {
            ...
            case 1:
                ...
                fetch('/fund-types/')
                    ...
                    .then(data => {
                        console.log(data)
                        let options = data.map(type => `<option value="${type}">${type}</option>`).join('');
                        recommendationSteps.innerHTML = `
                ...
                fetch('/risk-profiles/')
                    ...
                    console.log(data)
                        let radios = data.map(profile => `
                            <label class="radio-label"><input type="radio" name="risk-profile" value="${profile.id}"> ${profile}</label>
                        `).join('');
                        recommendationSteps.innerHTML = `
    ...
    function saveUserPreferences(preferences) {
        ...
        themes: JSON.stringify(preferences.themes),  // Already an array
